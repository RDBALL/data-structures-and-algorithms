...
const screenForNames = (arr) => {
  // Solution code here...
  let prefixOrTitle = /^(Mr?s?.|Dr.) [a-zA-Z0-9]+/;
  return arr.filter(name => prefixOrTitle.test(name));
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  let bigFirstLetter = /^./;
  return arr.map(string => string.replace(bigFirstLetter,(firstLetter) => firstLetter.toUpperCase()));
};
